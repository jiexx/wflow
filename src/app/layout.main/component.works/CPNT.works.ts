...
@Component({
    templateUrl: './CPNT.works.html',
    styleUrls: ['./CPNT.works.css'],
})
export class CWorks extends Bus implements AfterViewInit {
    name(): string {
        return 'CWorks'
    }
    ...
    ngAfterViewInit(): void {
        ...
        this.user.getSchedulePage({limit:10000,skip:0, state:['cim','oim','uim'], fitlerCompleted:true}).then(data=>{
            if(data && data.page){
